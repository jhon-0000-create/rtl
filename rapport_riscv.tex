...
Le Chapitre~\ref{ch:arch} introduit l'architecture de plus haut niveau et
la hiérarchie des modules de la présente implémentation. Le
Chapitre~\ref{ch:pipeline} détaille chacun des cinq étages du pipeline.
Le Chapitre~\ref{ch:hazards} décrit la logique de gestion des aléas. Le
Chapitre~\ref{ch:impl} discute des conventions d'implémentation Verilog,
et le Chapitre~\ref{ch:verif} expose la méthodologie et les résultats de
la vérification. Le Chapitre~\ref{ch:discussion} aborde les limitations
et les optimisations envisageables, et le Chapitre~\ref{ch:conclusion}
conclut le rapport.
...
\begin{figure}[H]
  ...
  \caption{Schema bloc de plus haut niveau du processeur montrant les
  entrees-sorties de \texttt{mpt\_top}, les interfaces IMEM et DMEM,
  l'entree d'adresse de demarrage et la sortie \texttt{exception\_o}.}
  \label{fig:top-level}
\end{figure}
...
détection des aléas, décrit au Chapitre~\ref{ch:hazards}, active les
...
\begin{figure}[H]
  ...
  \caption{Detail de l'etage IF montrant le multiplexeur 4-vers-1
  d'entree du PC, la duplication \texttt{IMEM\_addr\_o}/\texttt{IF\_pc\_o}
  et le multiplexeur d'injection de NOP pilote par \texttt{flush\_i}.}
  \label{fig:if-stage}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Architecture interne de l'etage ID montrant le decodeur, le
...
\begin{figure}[H]
  ...
  \caption{Architecture interne de l'etage EX montrant les multiplexeurs
  d'operande a 3 voies, l'ALU, le generateur de branchement et la
  propagation des signaux de controle vers la frontiere EX/MEM.}
  \label{fig:ex-stage}
\end{figure}
...
Tableau~\ref{tab:alu-ops}. Onze opérations couvrent l'ensemble du
sous-jeu RV32I.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{fig3.4.png}
  \caption{Vue interne de \texttt{mpt\_branchgen} faisant apparaitre le
...
Tableau~\ref{tab:byteen} récapitule les masques générés selon le couple
...
Cette logique sera examinée plus en détail au Chapitre~\ref{ch:hazards}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{fig3.5.png}
  \caption{Vue de l'etage MEM montrant la decomposition adresse/donnee,
  les generateurs de \texttt{DMEM\_wr\_byte\_en\_o}, l'unite d'extraction
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{fig3.6.png}
  \caption{Architecture de l'etage WB montrant le multiplexeur 2-vers-1
  pilote par \texttt{MEM\_memtoreg\_i} et le chemin de retour vers le
  banc de registres.}
  \label{fig:wb-stage}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Diagramme de chronogramme illustrant un alea charge-utilisation :
  trois cycles successifs montrant le \texttt{LW}, la bulle inseree, et le
  renvoi MEM/WB resolvant la dependance.}
...
(Section~\ref{sec:ex}) qui efface alors les autorisations destructives.
...
\begin{itemize}
  \item le renvoi WR-RD interne à \texttt{mpt\_regfile}
        (Section~\ref{sec:regfile}) ;
  \item le renvoi local au générateur de branchement,
        \texttt{mpt\_branchgen} (Section~\ref{sec:branchgen}).
\end{itemize}
...
Le Tableau~\ref{tab:tests} présente la liste complète des tests avec leur
...
\begin{figure}[H]
  ...
  \caption{Capture d'ecran de la sortie du testbench en mode
  \texttt{+runall} affichant les 42 verdicts et le
  recapitulatif final.}
...
\begin{figure}[H]
  ...
  montrant la montee de \texttt{x3}, \texttt{x17} et \texttt{x10} aux
  valeurs de signature de succes.}
  \label{fig:gtkwave}
\end{figure}
